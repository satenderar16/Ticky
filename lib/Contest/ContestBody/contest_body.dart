          ...
          if (submitText != ContestSubmission.none) {
            return BottomSubmitNavbar(
              onSubmit: () async {
                ...
                await entryNotifier.submitAnswers();
              },
            );
          }
  ...
  Widget build(BuildContext context) {
    return Consumer(
      builder: (context, ref, child) {
        ...
        final remaining = endAt!.difference(clock);
        ...
        if (remaining.inSeconds == 0) {
          Future.microtask(() {
            ref
                ...
                .contestSubmission(ContestSubmission.timeOutSumit);
          });
        }
...
class _QuestionFullViewState extends ConsumerState<QuestionFullView> {
  @override
  void initState() {
    // ensuring the timer start when first question frame builds:
    Future.microtask(() {
